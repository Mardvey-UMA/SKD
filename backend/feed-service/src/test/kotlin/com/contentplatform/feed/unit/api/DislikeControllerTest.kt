...
import com.contentplatform.feed.api.controller.DislikeController
...
class DislikeControllerTest {
    ...
    @Nested
    inner class `POST api feed dislikes` {

        @Test
        fun `should return 201 when dislike added successfully`() {
            val userId = UUID.randomUUID().toString()
            val contentId = UUID.randomUUID()

            every { collectionService.addDislike(UUID.fromString(userId), contentId) } just runs

            mockMvc.perform(authenticatedRequest("POST", "/api/feed/dislikes/$contentId", userId = userId))
                .andExpect(status().isCreated)
                .andExpect(jsonPath("$.content_id", `is`(contentId.toString())))
                .andExpect(jsonPath("$.action", `is`("disliked")))
        }

        @Test
        fun `should return 409 when dislike already exists`() {
            val userId = UUID.randomUUID().toString()
            val contentId = UUID.randomUUID()

            every { collectionService.addDislike(UUID.fromString(userId), contentId) } throws
                AlreadyExistsException("Dislike already exists")
    ...
    @Nested
    inner class `DELETE api feed dislikes` {

        @Test
        fun `should return 200 when dislike removed successfully`() {
            val userId = UUID.randomUUID().toString()
            val contentId = UUID.randomUUID()

            every { collectionService.removeDislike(UUID.fromString(userId), contentId) } just runs

            mockMvc.perform(authenticatedRequest("DELETE", "/api/feed/dislikes/$contentId", userId = userId))
                .andExpect(status().isOk)
                .andExpect(jsonPath("$.content_id", `is`(contentId.toString())))
                .andExpect(jsonPath("$.action", `is`("undisliked")))
        }
    }

    @Nested
    inner class `GET api feed dislikes` {

        @Test
        fun `should return 200 with paginated dislikes list`() {
            val userId = UUID.randomUUID().toString()
            val item = ContentBatchItem(id = "content-1", title = "Disliked Article")
            ...
            every { collectionService.getDislikes(UUID.fromString(userId), null) } returns pageResponse

            mockMvc.perform(authenticatedRequest("GET", "/api/feed/dislikes", userId = userId))
                .andExpect(status().isOk)
